...
namespace Negocio
{
    public class ModificoStockNegocio
    {
        public List<Stock> Listamodificarstock(int id)
        {
            ...
            try
            {
                dato1.setearConsulta("select s.ID, p.DESCRIPCION, s.FECHA_INGRESO, s.CANTIDAD, s.STOCK, s.ESTADO from stockproductos s inner join Productos p on s.IDProducto=p.ID where s.id =" + id);
